...
class Player:
    ...
    def Damage(self, EnemyN):
        if random.randint(1,100) <= EnemyN.Eva:
            if self.PlayerType == "Rouge" and self.Gaurdian == 1:
                print("Too Fast:\nYour Enemy is going to dodge so you push your body to go even faster completeing the attack")
                damage = random.randint((self.Att-1),(self.Att+2))
                if EnemyN.Block == 1:
    ...
    def SA(self, EnemyN):
        if self.PlayerType == "Warrior":
            multi = self.MP
            self.MP = 0
            Damage = int(((random.randint((self.Att),(self.Att+3)))*(multi*.65)//1))
            if Damage < 0:
                Damage = 0
            EnemyN.H = (EnemyN.H-Damage)
            print(f"You use all you power for one attack dealing {Damage} damage but your attack has been halfed\n")
            ...
            if Damage < 0:
                Damage = 0
            EnemyN.H = (EnemyN.H-Damage)
            print(f"You lift a rock launching it at the enemy dealing {Damage} damage, and stunning the enemy for a turn\n")
            EnemyN.Stun = 1
            ...
            if Damage < 0:
                Damage = 0
            EnemyN.H = (EnemyN.H-Damage)
            print(f"You dash past the enemy catching them off guard dealing {Damage} damage and doubling your evasion\n")
...
class Enemy:
    ...
    def SA(self, PlayerN):
        if self.EnemyType == "Skeletion Mage":
            ...
            self.H = self.H-4
            damage = int((random.randint((self.Att),(self.Att+2))*1.5)//1)
            if PlayerN.Block == 1:
            ...
            multi = self.MP
            self.MP = 0
            damage = int((random.randint((self.Att-2),(self.Att+2))*(1+(.1*multi)))//1)
            if PlayerN.Block == 1:
                block = ((int(PlayerN.Def*1.5)//1))
                block = random.randint((block-1),(block))
            else:
...
#Set version to reset stats
SetWarrior =       Player(13,13,4, 4, 4,2,3,5,0,0,"Warrior",1)
SetMage =          Player(10,10,10,10,3,3,4,10,0,0,"Mage",1)
SetRouge =         Player(8, 8, 4, 4, 5,1,8,25,0,0,"Rouge",1)
SetSkeleton_Mage = Enemy(12, 12,6, 6, 3,1,3,8,0,0,"Skeletion Mage")
SetOgre =          Enemy(17, 17,0, 0, 3,1,0,3,0,0,"Ogre")
SetZombie_Dog =    Enemy(7,  7, 4, 4, 4,0,7,20,0,0,"Zombie Dog")
